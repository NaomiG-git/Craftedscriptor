...
// Simple in-memory no-repeat decks per niche
const _decks = {};

window.nextPrompt = function (niche) {
  const list = (window.PROMPTS[niche] || []).slice(); // copy
  if (!list.length) return null;

  if (!_decks[niche] || _decks[niche].length === 0) {
    // shuffle new deck
    _decks[niche] = list.sort(() => Math.random() - 0.5);
  }

  return _decks[niche].pop();
};
